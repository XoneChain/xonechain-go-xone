...
FROM xonemaster/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gxone --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gxone --networkid {{.NetworkID}} --port {{.NodePort}} --xoneboots {{.Xoneboots}} --ethstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/XonemasterNetworkID/{{.NetworkID}}/g' dist/js/etherwallet-master.js && \
	sed -i 's/XonemasterNetwork/{{.Network}}/g'     dist/js/etherwallet-master.js && \
	sed -i 's/XonemasterDenom/{{.Denom}}/g'         dist/js/etherwallet-master.js && \
	sed -i 's/XonemasterHost/{{.Host}}/g'           dist/js/etherwallet-master.js && \
	sed -i 's/XonemasterRPCPort/{{.RPCPort}}/g'     dist/js/etherwallet-master.js
...
func deployWallet(client *sshClient, network string, xoneboots []string, config *walletInfos, nocache bool) ([]byte, error) {
