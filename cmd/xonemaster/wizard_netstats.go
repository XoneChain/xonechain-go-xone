...
func (w *wizard) networkStats() {
	...
	w.conf.ethstats = ""
	w.conf.xoneboots = w.conf.xoneboots[:0]
...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for xoneboot availability")
	if infos, err := checkNode(client, w.network, true); err != nil {
		if err != ErrServiceUnknown {
			stat.services["xoneboot"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["xoneboot"] = infos.Report()

		genesis = string(infos.genesis)
		xoneboots = append(xoneboots, infos.enode)
	}
	...
	w.conf.xoneboots = append(w.conf.xoneboots, xoneboots...)
