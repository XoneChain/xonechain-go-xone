...
{{end}}
RUN \
  echo 'gxone --cache 512 init /genesis.json' > gxone.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.xonechain/keystore/ && cp /signer.json /root/.xonechain/keystore/' >> gxone.sh && \{{end}}
	echo $'gxone --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Xoneboots}}--xoneboots {{.Xoneboots}}{{end}} {{if .Etherbase}}--etherbase {{.Etherbase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> gxone.sh

ENTRYPOINT ["/bin/sh", "gxone.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Xonechain node (xoneboot or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
func deployNode(client *sshClient, network string, xoneboots []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.etherbase == "" {
		kind = "xoneboot"
		xoneboots = make([]string, 0)
	}
	...
	template.Must(template.New("").Parse(nodeDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"LightFlag": lightFlag,
		"Xoneboots": strings.Join(xoneboots, ","),
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	// Inspect a possible xoneboot container on the host
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gxone --exec admin.nodeInfo.id attach", network, kind)); err != nil {
