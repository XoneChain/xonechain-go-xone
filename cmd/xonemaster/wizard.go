...
// config contains all the configurations needed by xonemaster that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	xoneboots []string // Xoneboots to always connect to by all nodes
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save xonemaster configs", "file", c.path, "err", err)
	}
}
