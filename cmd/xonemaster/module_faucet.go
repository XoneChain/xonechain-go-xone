...
FROM xonechain/client-go:alltools-latest
...
ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--xoneboots", "{{.Xoneboots}}", "--ethstats", "{{.Ethstats}}", "--ethport", "{{.EthPort}}",     \
...
func deployFaucet(client *sshClient, network string, xoneboots []string, config *faucetInfos, nocache bool) ([]byte, error) {
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Xonechain listener port":      strconv.Itoa(info.node.port),
