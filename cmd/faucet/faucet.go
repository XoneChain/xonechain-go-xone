...
var (
	...
	bootFlag    = flag.String("xoneboots", "", "Comma separated xoneboot enode URLs to seed with")
...
func main() {
	...
	// Convert the xoneboots to internal enode representations
	var enodes []*discv5.Node
	for _, boot := range strings.Split(*bootFlag, ",") {
		if url, err := discv5.ParseNode(boot); err == nil {
			enodes = append(enodes, url)
		} else {
			log.Error("Failed to parse xoneboot URL", "url", boot, "err", err)
		}
	}
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Boot up the client and ensure it connects to xoneboots
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/xonechain/go-xone/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
