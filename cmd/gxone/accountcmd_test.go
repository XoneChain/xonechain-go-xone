...
func TestAccountListEmpty(t *testing.T) {
	gxone := runGxone(t, "account", "list")
	gxone.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gxone := runGxone(t, "account", "list", "--datadir", datadir)
	defer gxone.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gxone := runGxone(t, "account", "new", "--lightkdf")
	defer gxone.ExpectExit()
	gxone.Expect(`
...
`)
	gxone.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gxone := runGxone(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	gxone := runGxone(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gxone.ExpectExit()
	gxone.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gxone.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gxone.StderrText(), m) {
...
// https://github.com/xonechain/go-xone/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gxone.SetTemplateFunc("keypath", func(file string) string {
