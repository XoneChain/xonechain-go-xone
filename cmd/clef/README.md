Clef
----
Clef can be used to sign transactions and data and is meant as a replacement for gxone's account management.
This allows DApps not to depend on gxone's account management. When a DApp wants to sign data it can send the data to
...
The general flow for signing a transaction using e.g. gxone is as follows:
![image](sign_flow.png)

In this case, `gxone` would be started with `--externalsigner=http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
put together is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: Gxone should be started in `gxone --external_signer localhost:8550`.
    - Currently, the Gxone APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This
  ...
  retains the original input.
    - The Gxone api should switch to use the same type, and relay `to`-account verbatim to the external api.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus gxone/mist/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
...
The signer listens to HTTP requests on `rpcaddr`:`rpcport`, with the same JSONRPC standard as Gxone. The messages are
