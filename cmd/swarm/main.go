...
import (
	...
	"github.com/xonechain/go-xone/swarm"
	bzzapi "github.com/xonechain/go-xone/swarm/api"
	swarmmetrics "github.com/xonechain/go-xone/swarm/metrics"
...
// This init function sets defaults so cmd/swarm can run alongside gxone.
func init() {
...
func bzzd(ctx *cli.Context) error {
	...
	//gxone only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for gxone and swarm
	...
	// Add xoneboots as initial peers.
	if bzzconfig.XoneBoots != "" {
		xoneboots := strings.Split(bzzconfig.XoneBoots, ",")
		injectXoneboots(stack.Server(), xoneboots)
	} else {
		if bzzconfig.NetworkId == 3 {
			injectXoneboots(stack.Server(), testbetXoneBoots)
		}
	}
...
func injectXoneboots(srv *p2p.Server, nodes []string) {
	for _, url := range nodes {
		n, err := discover.ParseNode(url)
		if err != nil {
			log.Error("Invalid swarm xoneboot", "err", err)
			continue
		}
		srv.AddPeer(n)
	}
