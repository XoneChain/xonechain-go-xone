...
const (
	...
	SWARM_ENV_XONEBOOTS       = "SWARM_XONEBOOTS"
	GXONE_ENV_DATADIR         = "GXONE_DATADIR"
)
...
var tomlSettings = toml.Config{
	...
	MissingField: func(rt reflect.Type, field string) error {
		...
		if unicode.IsUpper(rune(rt.Name()[0])) && rt.PkgPath() != "main" {
			link = fmt.Sprintf(", check github.com/xonechain/go-xone/swarm/api/config.go for available fields")
		}
...
func initSwarmNode(config *bzzapi.Config, stack *node.Node, ctx *cli.Context) {
	...
	//set the resolved config path (gxone --datadir)
...
func cmdLineOverride(currentConfig *bzzapi.Config, ctx *cli.Context) *bzzapi.Config {
	...
	if ctx.GlobalIsSet(utils.XonebootsFlag.Name) {
		currentConfig.XoneBoots = ctx.GlobalString(utils.XonebootsFlag.Name)
	}
...
func envVarsOverride(currentConfig *bzzapi.Config) (config *bzzapi.Config) {
	...
	if datadir := os.Getenv(GXONE_ENV_DATADIR); datadir != "" {
		currentConfig.Path = datadir
	}
	...
	if xoneboots := os.Getenv(SWARM_ENV_XONEBOOTS); xoneboots != "" {
		currentConfig.XoneBoots = xoneboots
	}
