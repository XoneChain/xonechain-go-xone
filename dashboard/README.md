...
The dashboard is a data visualizer integrated into gxone, intended to collect and visualize useful information of an Xonechain node. It consists of two parts:
...
Normally the dashboard assets are bundled into Gxone via `go-bindata` to avoid external dependencies. Rebuilding Gxone after each UI modification however is not feasible from a developer perspective. Instead, we can run `yarn dev` to watch for file system changes and refresh the browser automatically.

```
$ gxone --dashboard --vmodule=dashboard=5
$ (cd dashboard/assets && yarn dev)
```

To bundle up the final UI into Gxone, run `go generate`:
