...
import (
	...
	"github.com/xonechain/go-xone/accounts/usbwallet/internal/trezor"
...
// or whether the Xonechain app was not started on it.
...
// Xonechain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.XonechainAddress)
	if _, err := w.trezorExchange(&trezor.XonechainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.XonechainSignTx{
	...
	response := new(trezor.XonechainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.XonechainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
