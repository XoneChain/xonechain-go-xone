...
#
# The cluster contains a xoneboot, a gxone node and multiple swarm nodes, with
...
# static xoneboot configuration
XONEBOOT_IP="192.168.33.2"
XONEBOOT_PORT="30301"
XONEBOOT_KEY="32078f313bea771848db70745225c52c00981589ad6b5b49163f0f5ee852617d"
XONEBOOT_PUBKEY="760c4460e5336ac9bbd87952a3c7ec4363fc0a97bd31c86430806e287b437fd1b01abc6e1db640cf3106b520344af1d58b00b57823db3e1407cbc433e1b6d04d"
XONEBOOT_URL="enode://${XONEBOOT_PUBKEY}@${XONEBOOT_IP}:${XONEBOOT_PORT}"

# static gxone configuration
GXONE_IP="192.168.33.3"
GXONE_RPC_PORT="8545"
GXONE_RPC_URL="http://${GXONE_IP}:${GXONE_RPC_PORT}"
...
# start_xoneboot starts a xoneboot which is used to bootstrap the gxone and
# swarm nodes
start_xoneboot() {
  local key_file="${base_dir}/xoneboot.key"
  echo -n "${XONEBOOT_KEY}" > "${key_file}"

  local args=(
    --addr      "${XONEBOOT_IP}:${XONEBOOT_PORT}"
    --nodekey   "${key_file}"
    --verbosity "6"
  )

  start_node "xoneboot" "${XONEBOOT_IP}" "$(which xoneboot)" ${args[@]}
}

# start_gxone_node starts a gxone node with --datadir pointing at <base-dir>/gxone
# and a single, unlocked account with password "gxone"
start_gxone_node() {
  local dir="${base_dir}/gxone"
  mkdir -p "${dir}"

  local password="gxone"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating gxone account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gxone account address"
  fi

  local args=(
    --datadir   "${dir}"
    --networkid "321"
    --xoneboots "${XONEBOOT_URL}"
    --unlock    "${address}"
    --password  "${dir}/password"
    --rpc
    --rpcaddr   "${GXONE_IP}"
    --rpcport   "${GXONE_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gxone" "${GXONE_IP}" "$(which gxone)" ${args[@]}
}
...
create_account() {
  ...
  gxone --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
