...
import (
	...
	gxonemetrics "github.com/xonechain/go-xone/metrics"
	"github.com/xonechain/go-xone/metrics/influxdb"
...
func Setup(ctx *cli.Context) {
	if gxonemetrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(gxonemetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", map[string]string{
				"host": hosttag,
			})
		}
	}
