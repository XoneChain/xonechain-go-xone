...
// Package gxone contains the simplified mobile APIs to go-xone.
//
// The scope of this package is *not* to allow writing a custom Xonechain client
// with pieces plucked from go-xone, rather to allow writing native dapps on
...
// from the go-xone repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-xone version.
